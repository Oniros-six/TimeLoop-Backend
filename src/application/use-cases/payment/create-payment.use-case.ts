...
export class CreatePayment {
  ...
  async execute(
    ...
    try {
      ...
      const previousPayment = await this.paymentRepository.findByBookingId(
        booking.id,
      );
      const hasApproved = previousPayment.some(
      ...
      if (securityContext) {
        ...
        const securityViolations =
          this.securityValidator.validatePaymentSecurity(securityContextData);
      ...
      const paymentResult =
        await this.paymentOrchestrator.processPaymentWithProvider(savedPayment);
