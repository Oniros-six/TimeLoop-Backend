...
export class UpdateSent {
    ...
    async execute(invoiceId: number) {
        ...
        //* Actualizar invoice
        const result = await this.invoiceRepository.updateStatus({ invoiceId: invoiceId, status: InvoiceStatus.SENT });
        ...
        await this.activityLogService.updated({
            ...
            customerId: null,
            detail: `Factura actualizada con exito para el comercio con id: ${result.commerceId}, estado: ${InvoiceStatus.SENT}`,
        });
