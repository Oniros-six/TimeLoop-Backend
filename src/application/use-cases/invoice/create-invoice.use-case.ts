...
export class CreateInvoice {
  ...
  async execute() {
    ...
    const highFeePercentage = this.configService.get<number>(
      'HIGH_FEE_PERCENTAGE',
      0.01,
    );
    const lowFeePercentage = this.configService.get<number>(
      'LOW_FEE_PERCENTAGE',
      0.005,
    );
    ...
    for (const commerce of commerces) {
      try {
        ...
        const historyFound =
          await this.bookingHistoryRepository.findByDatesAndCommerce({
        ...
        const totalCanceled = historyFound.filter(
          (h) => h.status === BookingStatus.CANCELED,
        ).length;
        const completedBookings = historyFound.filter(
          (h) => h.status === BookingStatus.COMPLETED,
        );
        const totalIncome = completedBookings.reduce(
          (total, b) => total + b.priceAtBooking,
          0,
        );

        //* Fee aplicado
        const appliedFee = calculateFee(
          totalIncome,
          lowIncome,
          highIncome,
          lowFeePercentage,
    ...
    if (results.length === 0) {
      return {
        message:
          'No se encontró historial de facturación para comercios activos.',
        statusCode: HttpStatus.NOT_FOUND,
      };
    }
