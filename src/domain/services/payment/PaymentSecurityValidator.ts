...
export interface SecurityViolation {
    type: 'RATE_LIMIT' | 'SUSPICIOUS_AMOUNT' | 'MULTIPLE_FAILURES' | 'UNUSUAL_PATTERN';
...
export class PaymentSecurityValidator {
    private readonly rateLimitMap = new Map<string, { count: number; lastReset: Date }>();
    private readonly failureCountMap = new Map<string, { count: number; lastFailure: Date }>();
    ...
    private loadSecurityConfig(): SecurityConfig {
        return {
            rateLimit: {
                windowMs: parseInt(process.env.PAYMENT_RATE_LIMIT_WINDOW_MS || '900000'), // 15 minutos por defecto
                maxAttempts: parseInt(process.env.PAYMENT_RATE_LIMIT_MAX_ATTEMPTS || '10'),
            },
            failures: {
                maxFailuresPerHour: parseInt(process.env.PAYMENT_MAX_FAILURES_PER_HOUR || '5'),
            },
            suspiciousAmount: {
                threshold: parseFloat(process.env.PAYMENT_SUSPICIOUS_AMOUNT_THRESHOLD || '0.3'), // 30%
     ...
     */
    async validatePaymentSecurity(context: SecurityValidationContext): Promise<SecurityViolation[]> {
     ...
     */
    private validateRateLimit(context: SecurityValidationContext): SecurityViolation | null {
        ...
        if (now.getTime() - rateLimitData.lastReset.getTime() > this.config.rateLimit.windowMs) {
            this.rateLimitMap.set(key, { count: 1, lastReset: now });
            return null;
        }
     ...
     */
    private validateSuspiciousAmount(context: SecurityValidationContext): SecurityViolation | null {
     ...
     */
    private validateMultipleFailures(context: SecurityValidationContext): SecurityViolation | null {
     ...
     */
    private validateUnusualPatterns(context: SecurityValidationContext): SecurityViolation | null {
        const hour = new Date().getHours();
        
        // Detectar pagos en horarios inusuales configurados
        if (hour >= this.config.unusualPatterns.unusualHours.start && 
