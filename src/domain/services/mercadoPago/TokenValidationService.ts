...
export class TokenValidationService {
    ...
    async validateAndRefreshToken(commerceId: number): Promise<string> {
        try {
            const credentials = await this.mercadoPagoRepository.findByCommerceId(commerceId);
            ...
            if (timeUntilExpiration <= fiveMinutesInMs) {
                console.log(`Token para comercio ${commerceId} expira pronto, refrescando...`);
            ...
            throw new Error(`Error validando token para comercio ${commerceId}: ${message}`);
        }
    }
     ...
     */
    private async refreshToken(credentials: any): Promise<string> {
        try {
            const newTokens = await this.mercadoPagoService.refreshAccessToken(credentials.refreshToken);
            
            // Crear el token en el formato esperado por el repositorio
            const tokenInstance = {
                commerceId: credentials.commerceId,
                accessToken: newTokens.accessToken,
                refreshToken: newTokens.refreshToken,
                publicKey: newTokens.publicKey,
                mpUserId: newTokens.mpUserId,
