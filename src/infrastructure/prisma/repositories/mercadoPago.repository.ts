...
export class PrismaMercadoPagoRepository implements IMercadoPagoRepository {
    ...
    async create(token: InstanceType<typeof DomainClient.Token>): Promise<DomainClient> {
        ...
        const encryptedAccessToken = this.encryptionService.encrypt(token.accessToken);
        const encryptedRefreshToken = this.encryptionService.encrypt(token.refreshToken);
        const encryptedPublicKey = token.publicKey ? this.encryptionService.encrypt(token.publicKey) : null;
    ...
    async findByCommerceId(commerceId: number): Promise<DomainClient> {
        ...
        const decryptedRefreshToken = this.encryptionService.decrypt(mp.refreshToken);
        const decryptedPublicKey = mp.publicKey ? this.encryptionService.decrypt(mp.publicKey) : null;
